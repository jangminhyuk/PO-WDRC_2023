...
#This method implements the SDP formualation of MMSE estimation problem from "Nguyen, Viet Anh, et al. "Bridging Bayesian and minimax mean square error estimation via Wasserstein distributionally robust optimization." Mathematics of Operations Research 48.1 (2023): 1-37.."
...
#        P_new = P_ - P_ @ self.C.T @ temp
    ...
    def solve_DR_sdp(self, P_t1, S_t1, M, X_cov, Sigma_hat, theta):
        ...
        # min_eig = np.real( np.min(np.linalg.eigvals(Sigma_z.value)) )
        # if np.all( np.linalg.eigvals(S.value - min_eig * np.eye(self.nx + self.ny)) > 0): # condition check!!
        #     pass
        # else:
        #     print(np.linalg.eigvals(S.value - min_eig * np.eye(self.nx + self.ny)))
        #     print("wrong!")
    ...
    def backward(self):
        ...
        for t in range(self.T):
            print("DRMMSE WDRC Offline step : ",t,"/",self.T)
            self.x_cov[t+1], self.S_opt[t+1], self.S_xx[t+1], self.S_xy[t+1], self.S_yy[t+1], self.sigma_wc[t] = self.DR_kalman_filter_cov(self.P[t+1], self.S[t+1], self.M_hat[t], self.x_cov[t], self.Sigma_hat[t])
