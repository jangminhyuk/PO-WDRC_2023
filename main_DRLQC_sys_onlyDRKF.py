...
def main(dist, noise_dist1, sim_type, num_sim, num_samples, num_noise_samples, T, method, plot_results, noise_plot_results, infinite, out_of_sample, wc, h_inf):
    ...
    for noise_dist in noisedist:
        for theta_drlqc in theta_drlqc_list:
            for theta in theta_v_list:
                for num_noise in num_noise_list:
                    print("disturbance : ", dist, "/ noise : ", noise_dist, "/ num_noise : ", num_noise, "/ theta_v : ", theta)
                #    enumerate([0.5, 1])            
                #    enumerate([0.00001, 0.0001, 0.0005, 0.0015, 0.001, 0.00015, 0.002, 0.0025, 0.005, 0.01, 0.015, 0.05, 0.1, 1]):
                    ...
                    if infinite:
                        ...
                        if sim_type == "multiple":
                            path = "./results/{}_{}/finite/multiple/theta={}/theta_drlqc={}/".format(dist, noise_dist,theta,theta_drlqc)
                        else:
            ...
            #                print(i)
                        ...
                        if infinite:
                            ...
                            drlqc = DRLQC(lambda_, theta_drlqc, T, dist, noise_dist, system_data, mu_hat, W_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, v_mean_hat, V_hat, tol)
                            
                            drlqc = LQG(T, dist, noise_dist, system_data, mu_hat, Sigma_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, v_mean_hat, M_hat)
            ...
            #            print('i: ', i)
                ...
                #        save_data(path + 'lqg_os.pkl', cost_lqg)
                        ...
                        if sim_type == "multiple":
                            summarize(output_lqg_list, output_wdrc_list, output_drkf_wdrc_list , output_drlqc_list, dist, noise_dist, path, num_sim, plot_results, True, True, True, application)
                        else:
                            for i in range(num_sim):
                                print('i: ', i)
                ...
                if noise_plot_results:
                    if infinite:
                        path = "./results/{}_{}/infinite/multiple/num_noise_plot/{}/theta_drlqc={}/".format(dist, noise_dist, theta, theta_drlqc)
                    else:
                        path = "./results/{}_{}/finite/multiple/num_noise_plot/{}/theta_drlqc={}/".format(dist, noise_dist, theta, theta_drlqc)
                    if not os.path.exists(path):
                        os.makedirs(path)
