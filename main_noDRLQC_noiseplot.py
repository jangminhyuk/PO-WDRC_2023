...
def main(dist, noise_dist1, sim_type, num_sim, num_samples, num_noise_samples, T, method, plot_results, noise_plot_results, infinite, out_of_sample, wc, h_inf):
    ...
    output_J_LQG_mean, output_J_WDRC_mean, output_J_DRKF_WDRC_mean, output_J_DRLQC_mean =[], [], [], []
    output_J_LQG_std, output_J_WDRC_std, output_J_DRKF_WDRC_std, output_J_DRLQC_std =[], [], [], [] 
    #-------Initialization-------
    ...
    #----------------------------
    #theta_w =0.1
    theta_v_list = [1.0, 1.2, 1.4, 1.6, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]
    #theta_v_list = [4.0, 5.0, 6.0, 7.0, 8.0] # erase this!!
    theta_v_list = [2.0]
        ...
        for theta_w in theta_w_list:
            ...
            #                print(i)
                        ...
                        if infinite:
                            ...
                            #DRKF method
                            drkf_wdrc = DRKF_WDRC_test(lambda_, theta_w, theta, theta_x0, T, dist, noise_dist, system_data, mu_hat, Sigma_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, v_mean_hat,  M_hat, -1)
                            ...
                            drlqc = DRLQC(theta_w, theta, theta_x0, T, dist, noise_dist, system_data, mu_hat, W_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, v_mean_hat, V_hat, tol)
            ...
            #            print('i: ', i)
                ...
                #        save_data(path + 'lqg_os.pkl', cost_lqg)
                        ...
                        J_LQG_list, J_WDRC_list, J_DRKF_WDRC_list, J_DRLQC_list = [], [], [], []
                        ...
                        # print("Average cost (LQG) : ", J_LQG_mean[0], " Average cost (WDRC) : ", J_WDRC_mean[0], " Average cost (DRKF-WDRC) : ", J_DRKF_WDRC_mean[0], " Average cost (DRLQC) : ", J_DRLQC_mean[0])
                        # print("std (LQG) : ", J_LQG_std[0], " std (WDRC) : ", J_WDRC_std[0], " std (DRKF-WDRC) : ", J_DRKF_WDRC_std[0], " std (DRLQC) : ", J_DRLQC_std[0])
                    else:
                        #Save results
                        ...
                        save_data(path + 'wdrc_' + theta_w_ + '.pkl', J_WDRC_mean)
                        #save_data(path + 'drlqc_' + theta_w_ + 'and' + theta_v_+ '.pkl', J_DRLQC_mean)
                        ...
                        print("theta_w : ", theta_w, " / theta_v : ", theta_v_)
                ...
                if noise_plot_results:
                    ...
                    theta_w_ = f"_{str(theta_w).replace('.', '_')}" # change 1.0 to 1_0 for file name
                    theta_v_ = f"_{str(theta).replace('.', '_')}" # change 1.0 to 1_0 for file name
                    save_data(path + 'drkf_wdrc_mean' + theta_w_ + ' and' + theta_v_ + '.pkl', output_J_DRKF_WDRC_mean)
                    save_data(path + 'drkf_wdrc_std' + theta_w_ + ' and' + theta_v_ + '.pkl', output_J_DRKF_WDRC_std)  
                    save_data(path + 'lqg_mean.pkl', output_J_LQG_mean)
                    save_data(path + 'lqg_std.pkl', output_J_LQG_std) 
                    save_data(path + 'wdrc_mean' + theta_w_ + '.pkl', output_J_WDRC_mean)
                    save_data(path + 'wdrc_std' + theta_w_ + '.pkl', output_J_WDRC_std) 
                    # save_data(path + 'drlqc_mean' + theta_w_ + ' and' + theta_v_ + '.pkl', output_J_DRLQC_mean)
                    # save_data(path + 'drlqc_std' + theta_w_ + ' and' + theta_v_ + '.pkl', output_J_DRLQC_std) 
                    
                    
                    #Summarize and plot the results
                    print('\n-------Summary-------')
                    print("dist : ", dist, "noise_dist : ", noise_dist, "/ num_disturbance_samples : ", num_samples, "/ theta : ", theta, " / noise sample effect PLOT / Seed : ",seed)
                    # summarize_noise(num_noise_list, output_J_LQG_mean, output_J_LQG_std, output_J_WDRC_mean, output_J_WDRC_std, output_J_DRKF_WDRC_mean, output_J_DRKF_WDRC_std, output_J_DRLQC_mean, output_J_DRLQC_std, dist, noise_dist, path, False)
                    # output_J_DRLQC_mean, output_J_DRKF_WDRC_mean, output_J_LQG_mean, output_J_WDRC_mean = [], [], [], []
                    # output_J_DRLQC_std, output_J_DRKF_WDRC_std, output_J_LQG_std, output_J_WDRC_std = [], [], [], []
