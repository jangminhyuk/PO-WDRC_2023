...
def main(dist, noise_dist1, sim_type, num_sim, num_samples, num_noise_samples, T, method, plot_results, noise_plot_results, infinite, out_of_sample, wc, h_inf):
    ...
    for theta_w in theta_w_list:
        for noise_dist in noisedist:
            for theta in theta_list: # theta indicates theta_v
                for num_noise in num_noise_list:
                    ...
                    if infinite:
                        ...
                        if sim_type == "multiple":
                            path = "./results/{}_{}/finite/multiple/theta_w={}/".format(dist, noise_dist,theta)
                        else:
            ...
            #                print(i)
                        ...
                        if infinite:
                            ...
                            #DRKF method
                            #drkf_wdrc = DRKF_WDRC_test(theta_w, theta, T, dist, noise_dist, system_data, mu_hat, Sigma_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, v_mean_hat,  M_hat, -1)
