...
def main(dist, noise_dist1, sim_type, num_sim, num_samples, num_noise_samples, T, method, plot_results, dist_plot_results, infinite, out_of_sample, wc, h_inf):
    application = "Nothing"
    lambda_ = 1634
    seed = 2024 # any value
    if dist_plot_results: # if you need to draw ploy_J
        num_dist_list = [3,5,7,9,11,13,15]
    else:
        num_dist_list = [num_noise_samples]
    ...
    for theta_w in theta_w_list:
        for noise_dist in noisedist:
            for theta in theta_list:
                for num_dist in num_dist_list:
                    print("disturbance : ", dist, "/ noise : ", noise_dist, "/ num_noise : ", num_noise_samples, "/ theta_v : ", theta)
                #    enumerate([0.5, 1])            
                #    enumerate([0.00001, 0.0001, 0.0005, 0.0015, 0.001, 0.00015, 0.002, 0.0025, 0.005, 0.01, 0.015, 0.05, 0.1, 1]):
                                #:
                    np.random.seed(seed) # fix Random seed!
                    print("--------------------------------------------")
                    print("number of noise sample : ", num_noise_samples)
                    print("number of disturbance sample : ", num_dist)
                    #theta = 0.05
                    #Path for saving the results
                    if infinite:
                        if sim_type == "multiple":
                            if out_of_sample:
                                path = "./results/{}_{}/infinite/N={}/theta={}/".format(noise_dist,  num_dist, theta)
                            else:
            ...
            #            theta = 0.001 #Wasserstein ball radius
                    ...
                    if out_of_sample:
                        ...
                        for i in range(num_sim):
                            if infinite:
                                mu_hat_, Sigma_hat_ = gen_sample_dist_inf(dist, num_dist, mu_w=mu_w, Sigma_w=Sigma_w, w_max=w_max, w_min=w_min)
            #                    if dist=="normal":
                                mu_hat_ = 0*np.ones((nx, 1))
                            else:
                                mu_hat_, Sigma_hat_ = gen_sample_dist(dist, T, num_dist, mu_w=mu_w, Sigma_w=Sigma_w, w_max=w_max, w_min=w_min)
                            mu_hat.append(mu_hat_)
                            Sigma_hat.append(Sigma_hat_)
                    else:
                        if infinite:
                            mu_hat, Sigma_hat = gen_sample_dist_inf(dist, num_dist, mu_w=mu_w, Sigma_w=Sigma_w, w_max=w_max, w_min=w_min)
            #                if dist=="normal":
                            #Sigma_hat = Sigma_hat+ 1e-6*np.eye(nx)
                            mu_hat = 0*np.ones((nx, 1))
                            _, M_hat = gen_sample_dist_inf(noise_dist, num_noise_samples, mu_w=mu_v, Sigma_w=M, w_max=v_max, w_min=v_min) # generate M hat!
                        else:
                            mu_hat, Sigma_hat = gen_sample_dist(dist, T+1, num_dist, mu_w=mu_w, Sigma_w=Sigma_w, w_max=w_max, w_min=w_min)
            #                if dist=="normal":
                            #mu_hat = 0*np.ones((T, nx, 1))
                            v_mean_hat, M_hat = gen_sample_dist(noise_dist, T+1, num_noise_samples, mu_w=mu_v, Sigma_w=M, w_max=v_max, w_min=v_min) # generate M hat!(noise)
            ...
            #                print(i)
                            ...
                            while not wdrc_.flag or not lqg_.flag or not drkf_wdrc_.flag:
                                mu_hat[i], Sigma_hat[i] = gen_sample_dist(dist, T, num_dist, mu_w=mu_w, Sigma_w=Sigma_w, w_max=w_max, w_min=w_min)
                                drkf_wdrc_ = DRKF_WDRC(theta, T, dist, system_data, mu_hat[i], Sigma_hat[i], x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, M_hat)
                                wdrc_ = WDRC(theta, T, dist, system_data, mu_hat[i], Sigma_hat[i], x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, M_hat)
            ...
            #            print('i: ', i)
                ...
                #        save_data(path + 'lqg_os.pkl', cost_lqg)
                        ...
                        #Below should be removed!!!! just wrote DRKF instead of MMSE to prevent code error for a moment
                        for out in output_mmse_wdrc_list:
                            J_MMSE_WDRC_list.append(out['cost'])
                        ...
                        print("num_noise_sample : ", num_noise_samples, " / finished with dist : ", dist, "/ noise_dist : ", noise_dist, "/ seed : ", seed)
            ...
            #                        print('---------------------')
            #             else:
                        print("dist : ", dist,"/ noise dist : ", noise_dist, "/ num_samples : ", num_dist, "/ num_noise_samples : ", num_noise_samples, "/seed : ", seed)
                ...
                if dist_plot_results:
                    if infinite:
                        path = "./results/{}_{}/infinite/multiple/num_dist_plot/{}/".format(dist, noise_dist, theta)
                    else:
                        path = "./results/{}_{}/finite/multiple/num_dist_plot/{}/".format(dist, noise_dist, theta)
                    if not os.path.exists(path):
                        os.makedirs(path)
                    ...
                    #Summarize and plot the results
                    print('\n-------Summary-------')
                    print("dist : ", dist, "noise_dist : ", noise_dist, "/ num_disturbance_samples : ", num_dist, "/ theta : ", theta, " / noise sample effect PLOT / Seed : ",seed)
                    summarize_noise(num_dist_list, output_J_LQG_mean, output_J_LQG_std, output_J_WDRC_mean, output_J_WDRC_std, output_J_DRKF_WDRC_mean, output_J_DRKF_WDRC_std, output_J_MMSE_WDRC_mean, output_J_MMSE_WDRC_std, dist, noise_dist, path, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dist_plot', required=False, action="store_true") #plot plotJ_results, (effect of num_noise_samples)
    ...
    np.random.seed(100)
    main(args.dist, args.noise_dist, args.sim_type, args.num_sim, args.num_samples, args.num_noise_samples, args.horizon, args.method, args.plot, args.dist_plot, args.infinite, args.os, args.wc, args.h_inf)
