...
def summarize(lqg_theta_w_values, lqg_theta_v_values, lqg_cost_values ,wdrc_theta_w_values, wdrc_theta_v_values, wdrc_cost_values , drkf_theta_w_values, drkf_theta_v_values, drkf_cost_values, drlqc_theta_w_values, drlqc_theta_v_values, drlqc_cost_value, dist, noise_dist, path):
    surfaces = []
    labels = []
    # Create 3D plot
    plt.rcParams.update({
    ...
    surface_wdrc =ax.plot_surface(theta_w_grid_wdrc, theta_v_grid_wdrc, cost_grid_wdrc, alpha=0.3, color='blue', label='WDRC')
    ...
    theta_w_grid_drkf, theta_v_grid_drkf = np.meshgrid(
        np.linspace(min(drkf_theta_w_values), max(drkf_theta_w_values), 100),
        np.linspace(min(drkf_theta_v_values), max(drkf_theta_v_values), 100)
    )
    cost_grid_drkf = griddata(
        (drkf_theta_w_values, drkf_theta_v_values), drkf_cost_values,
        (theta_w_grid_drkf, theta_v_grid_drkf), method='cubic'
    )
    
    # Plot smooth surface - DRKF
    surface_drkf = ax.plot_surface(theta_w_grid_drkf, theta_v_grid_drkf, cost_grid_drkf, alpha=0.5, color='green', label='DRCE')
...
if __name__ == "__main__":
    ...
    #path = "./results_save/n_n_params_zeromean_10_10/"
    #Load data
    lqg_data =[]
    wdrc_data = []
    ...
    lqg_theta_v_values = []
    lqg_cost_values = []
    # theta_v_list = [2.5, 3.0, 3.5, 4.0, 4.5, 5.0, 6.0, 7.0]
    # theta_w_list = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
    theta_v_list = [2.3, 2.5, 3.0, 3.5, 4.0, 5.0]
    ...
    #pattern_drkf = r"drkf_wdrc__(\d+(?:\.\d+)?)_?(\d+(?:\.\d+)?)and_(\d+(?:\.\d+)?)_?(\d+(?:_\d+)?)?"
    #pattern_drkf  = r"drkf_wdrc__(\d+(?:\.\d+)?)_?(\d+(?:\.\d+)?)and_(\d+(?:\.\d+)?)_?(\d+(?:\.\d+)?)?"
    pattern_drkf = r"drkf_wdrc__(\d+(?:_\d+)?(?:\.\d+)?)_?(\d+(?:_\d+)?(?:\.\d+)?)?and_(\d+(?:_\d+)?(?:\.\d+)?)_?(\d+(?:_\d+)?(?:\.\d+)?)?"
    #pattern_drlqc = r"drlqc__(\d+(?:\.\d+)?)_?(\d+(?:\.\d+)?)and_(\d+(?:\.\d+)?)_?(\d+(?:\.\d+)?)?"
    pattern_drlqc = r"drlqc__(\d+(?:_\d+)?(?:\.\d+)?)_?(\d+(?:_\d+)?(?:\.\d+)?)?and_(\d+(?:_\d+)?(?:\.\d+)?)_?(\d+(?:_\d+)?(?:\.\d+)?)?" 
    pattern_wdrc = r"wdrc__(\d+(?:_\d+)?(?:\.\d+)?)_?(\d+(?:_\d+)?(?:\.\d+)?)?"
    pattern_lqg = r"lqg"
    ...
    for filename in os.listdir(path):
        ...
        if match:
            ...
            ##
            theta_w_value = float(match.group(1).replace('_', '.'))
            theta_v_value = float(match.group(3).replace('_', '.'))
            #print('theta_w : ', theta_w_value)
            #print('theta_v : ', theta_v_value, '\n')
            #changed _1_5_ to 1.5!
            #print('theta w : ', theta_w_value, ' / theta v :', theta_v_value)
            
            # Store theta_w and theta values
            drkf_theta_w_values.append(theta_w_value)
            ...
            if match_drlqc:
                ...
                #changed _1_5_ to 1.5!
                theta_w_value = float(match_drlqc.group(1).replace('_', '.'))
                theta_v_value = float(match_drlqc.group(3).replace('_', '.'))
                # Store theta_w and theta values
                ...
                if match_wdrc: # wdrc
                    # theta_w_value = float(match_wdrc.group(1))  # Extract theta w value
                    # theta_w_str = match_wdrc.group(2)
                    # theta_w_value += float(theta_w_str)/10
                    theta_w_value = float(match_wdrc.group(1).replace('_', '.'))
    ...
    drkf_theta_w_values = np.array(drkf_theta_w_values)
    ...
    summarize(lqg_theta_w_values, lqg_theta_v_values, lqg_cost_values ,wdrc_theta_w_values, wdrc_theta_v_values, wdrc_cost_values , drkf_theta_w_values, drkf_theta_v_values, drkf_cost_values ,drlqc_theta_w_values, drlqc_theta_v_values, drlqc_cost_values , args.dist, args.noise_dist, path)
